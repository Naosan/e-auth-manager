...
## 管理の主体（ライブラリ）
- 使用ライブラリ: `@naosan-internal/pipeline-kit`（本リポジトリにインストール済）
...
- 既定の暗号化JSON保存先を表示（Windows）
  - `node -e "import('dotenv/config'); import('@naosan-internal/pipeline-kit/file').then(({default:M})=>console.log(new M({ masterKey: process.env.EBAY_OAUTH_TOKEN_MANAGER_MASTER_KEY }).tokenFile))"`
